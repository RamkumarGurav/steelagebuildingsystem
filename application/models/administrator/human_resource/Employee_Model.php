...
class Employee_Model extends CI_Model
{
	...
	function get_employee($params = array())
	{
		...
		$this->db->join("designation_master as  dm", "dm.designation_id = ft.designation_id");
		...
		if (!empty($params['user_role_id'])) {
			$this->db->where("ft.user_role_id", $params['user_role_id']);
		}
		...
		if (!empty($result) && !empty($params['details'])) {
			foreach ($result as $admin_user) {
				...
				$this->db->where("aur.admin_user_id", $admin_user->admin_user_id);
				$admin_user->roles = $this->db->get()->result();

				// Fetch files for each admin_user
				$this->db->select("auf.*");
				$this->db->from("admin_user_file as auf");
				$this->db->where("auf.admin_user_id", $admin_user->admin_user_id);
