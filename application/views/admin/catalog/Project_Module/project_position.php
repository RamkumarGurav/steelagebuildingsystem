  ...
  <?php if ($user_access->view_module == 1) { ?>
    $(document).ready(function () {
      $.ajax({
        ...
        //dataType : "json",
        data: { "project_id": '<? echo $project_id; ?>', "withPosition": 1, 'sortByPosition': 1, "<?= $csrf['name'] ?>": "<?= $csrf['hash'] ?>" },
            ...
            <div class="tableDemo">
              <table class="table table-striped" id="table-2">
                <thead>
                  <tr>
                    <th>Slno.</th>
                    <th><input type="checkbox" name="main_check" id="main_check" onclick="check_uncheck_All_records()"
                        value="Checkbox" /><span style="display:none">Checkbox</span></th>
                    <th>Project Name</th>
              ...
              $("#table-2").tableDnD({
                ...
                onDrop: function (table, row) {
                  ...
                  $.ajax({
                    ...
                    //dataType : "json",
                    data: { "project_id": '<? echo $project_id; ?>', 'podId': podId, "<?= $csrf['name'] ?>": "<?= $csrf['hash'] ?>" },
                    success: function (result) {
                      // alert(result);
                      $('#projectList').html(result);
