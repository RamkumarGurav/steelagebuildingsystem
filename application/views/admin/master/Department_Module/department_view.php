            ...
            <div class="col-12">
                ...
                <div class="card">
                    ...
                    <div class="card-header">
                        <h3 class="card-title"><?=$department_master_data->department_name?></h3>
                        <div class="float-right">
                            <?php 
								if($user_access->add_module==1 && false)	{
								?>
								<a href="<?=MAINSITE_Admin.$user_access->class_name?>/department-edit"> 
							...
							?>
							<a href="<?=MAINSITE_Admin.$user_access->class_name?>/department-edit/<?=$department_master_data->department_id?>"> 
                    ...
                    <div class="card-body">
                            ...
                            <table id="" class="table table-bordered table-hover myviewtable responsiveTableNewDesign">
								...
								<tr>
										...
										<?=$department_master_data->department_id?></td>
										<td>
										<strong class="full">Department</strong>
										<?=$department_master_data->department_name?></td>
										<td >
										<strong class="full">Added On</strong>
										<?=date("d-m-Y h:i:s A" , strtotime($department_master_data->added_on))?></td>
										<td>
										<strong class="full">Added By</strong>
										<?=$department_master_data->added_by_name?></td>
										<td>
										<strong class="full">Updated On</strong>
										<? if(!empty($department_master_data->updated_on)){echo date("d-m-Y h:i:s A" , strtotime($department_master_data->updated_on));}else{echo "-";}?></td>
										<td>
										<strong class="full">Updated By</strong>
										<? if(!empty($department_master_data->updated_by_name)){echo $department_master_data->updated_by_name;}else{echo "-";}?></td>
										<td >
										<strong class="full">Status</strong>
										<? if($department_master_data->status==1){ ?> Active <i class="fas fa-check btn-success btn-sm "></i>
